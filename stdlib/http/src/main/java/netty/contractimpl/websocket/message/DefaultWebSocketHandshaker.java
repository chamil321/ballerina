...
package netty.contractimpl.websocket.message;
...
import netty.contract.websocket.ServerHandshakeFuture;
...
/**
 * Default implementation of {@link netty.contract.websocket.WebSocketHandshaker}.
 */
public class DefaultWebSocketHandshaker implements WebSocketHandshaker {

    private final ChannelHandlerContext ctx;
    private final FullHttpRequest httpRequest;
    private final netty.contract.ServerConnectorFuture connectorFuture;
    private final String target;
    private final boolean secureConnection;
    private boolean cancelled = false;
    private boolean handshakeStarted = false;
    private netty.message.HttpCarbonRequest request;

    public DefaultWebSocketHandshaker(ChannelHandlerContext ctx, ServerConnectorFuture connectorFuture,
                                      FullHttpRequest httpRequest) {
        this.ctx = ctx;
        this.connectorFuture = connectorFuture;
        this.secureConnection = ctx.channel().pipeline().get(netty.contract.Constants.SSL_HANDLER) != null;
    ...
    @Override
    public netty.contract.websocket.ServerHandshakeFuture handshake() {
        return handshake(null);
    }

    @Override
    public netty.contract.websocket.ServerHandshakeFuture handshake(String[] subProtocols) {
        return handshake(subProtocols, 0);
    }

    @Override
    public netty.contract.websocket.ServerHandshakeFuture handshake(String[] subProtocols, int idleTimeout) {
    ...
    public netty.contract.websocket.ServerHandshakeFuture handshake(String[] subProtocols, int idleTimeout,
        ...
        return handshake(subProtocols, idleTimeout, responseHeaders, netty.contract.Constants.WEBSOCKET_DEFAULT_FRAME_SIZE);
    }
    ...
    private String getWebSocketURL() {
        String scheme = netty.contract.Constants.WS_SCHEME;
        if (secureConnection) {
            scheme = netty.contract.Constants.WSS_SCHEME;
        }
    ...
    private void configureFrameHandlingPipeline(int idleTimeout, WebSocketInboundFrameHandler frameHandler) {
        ...
        pipeline.remove(netty.contract.Constants.WEBSOCKET_SERVER_HANDSHAKE_HANDLER);
        pipeline.remove(netty.contract.Constants.HTTP_CHUNK_WRITER);
        pipeline.remove(netty.contract.Constants.HTTP_EXCEPTION_HANDLER);
        ...
        if (idleTimeout > 0) {
            pipeline.replace(netty.contract.Constants.IDLE_STATE_HANDLER, netty.contract.Constants.IDLE_STATE_HANDLER,
                             new IdleStateHandler(0, 0, idleTimeout, TimeUnit.MILLISECONDS));
        } else {
            pipeline.remove(netty.contract.Constants.IDLE_STATE_HANDLER);
        }
    ...
    public netty.message.HttpCarbonRequest getHttpCarbonRequest() {
        return request;
    }
