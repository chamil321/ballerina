...
import netty.contract.websocket.WebSocketControlSignal;
import netty.contract.websocket.WebSocketFrameType;
import netty.contract.websocket.WebSocketTextMessage;
import netty.contractimpl.websocket.message.DefaultWebSocketCloseMessage;
import netty.contractimpl.websocket.message.DefaultWebSocketControlMessage;
...
public class WebSocketInboundFrameHandler extends ChannelInboundHandlerAdapter {
    ...
    private final netty.contract.websocket.WebSocketConnectorFuture connectorFuture;
    ...
    private netty.contract.websocket.WebSocketFrameType continuationFrameType;
    ...
    @Override
    public void channelInactive(ChannelHandlerContext ctx) throws netty.contract.websocket.WebSocketConnectorException {
        if (!caughtException && webSocketConnection != null && !closeFrameReceived && closePromise == null &&
            ...
            DefaultWebSocketMessage webSocketCloseMessage =
                    new netty.contractimpl.websocket.message.DefaultWebSocketCloseMessage(netty.contract.Constants.WEBSOCKET_STATUS_CODE_ABNORMAL_CLOSURE);
            setupCommonProperties(webSocketCloseMessage);
            connectorFuture.notifyWebSocketListener((netty.contract.websocket.WebSocketCloseMessage) webSocketCloseMessage);
            return;
        }
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (msg instanceof TextWebSocketFrame) {
            ...
            if (!textFrame.isFinalFragment()) {
                continuationFrameType = netty.contract.websocket.WebSocketFrameType.TEXT;
            }
            ...
            if (!binaryFrame.isFinalFragment()) {
                continuationFrameType = netty.contract.websocket.WebSocketFrameType.BINARY;
            }
            ...
            if (continuationFrameType == netty.contract.websocket.WebSocketFrameType.TEXT) {
    ...
    private void notifyPingMessage(PingWebSocketFrame pingWebSocketFrame) throws
                                                                          netty.contract.websocket.WebSocketConnectorException {
        netty.contract.websocket.WebSocketControlMessage webSocketControlMessage = WebSocketUtil.
                getWebSocketControlMessage(pingWebSocketFrame, netty.contract.websocket.WebSocketControlSignal.PING);
    ...
    private void notifyPongMessage(PongWebSocketFrame pongWebSocketFrame) throws
                                                                          netty.contract.websocket.WebSocketConnectorException {
        netty.contract.websocket.WebSocketControlMessage webSocketControlMessage = WebSocketUtil.
                getWebSocketControlMessage(pongWebSocketFrame, netty.contract.websocket.WebSocketControlSignal.PONG);
        setupCommonProperties((DefaultWebSocketMessage) webSocketControlMessage);
        connectorFuture.notifyWebSocketListener(webSocketControlMessage);
    }
