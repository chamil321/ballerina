...
import netty.contract.config.ForwardedExtensionConfig;
...
public class DefaultHttpClientConnector implements HttpClientConnector {
    ...
    public DefaultHttpClientConnector(ConnectionManager connectionManager, netty.contract.config.SenderConfiguration senderConfiguration,
        ...
        if (pushPromise.isRejected()) {
            outboundMsgHolder.getResponseFuture().
                    notifyPushResponse(pushPromise.getPromisedStreamId(),
                                       new netty.contract.exceptions.ClientConnectorException("Cannot fetch a response for an rejected promise",
    ...
    public HttpResponseFuture send(OutboundMsgHolder outboundMsgHolder, HttpCarbonMessage httpOutboundRequest) {
        ...
        try {
             ...
             */
            final netty.contractimpl.common.HttpRoute route = getTargetRoute(senderConfiguration.getScheme(), httpOutboundRequest);
            if (isHttp2) {
            ...
            targetChannel.getConnectionReadyFuture().setListener(new ConnectionAvailabilityListener() {
                ...
                private void prepareTargetChannelForHttp(ChannelFuture channelFuture) {
                    ...
                    netty.contractimpl.common.Util.setCorrelationIdForLogging(targetNettyChannel.pipeline(), targetChannel.getCorrelatedSource());

                    netty.contractimpl.common.Util.handleOutboundConnectionHeader(senderConfiguration, httpOutboundRequest);
                    String localAddress =
                            ((InetSocketAddress) targetNettyChannel.localAddress()).getAddress().getHostAddress();
                    netty.contractimpl.common.Util.setForwardedExtension(forwardedExtensionConfig, localAddress, httpOutboundRequest);
                }
    ...
    private netty.contractimpl.common.HttpRoute getTargetRoute(String scheme, HttpCarbonMessage httpCarbonMessage) {
