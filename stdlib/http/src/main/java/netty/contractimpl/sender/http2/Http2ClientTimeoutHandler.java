...
public class Http2ClientTimeoutHandler implements Http2DataEventListener {
    ...
    public class IdleTimeoutTask implements Runnable {
        ...
        private String getErrorMessage(boolean primary) {
            return primary ? netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_READING_INBOUND_RESPONSE_BODY :
                    netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_READING_PUSH_RESPONSE_BODY;
        }

        private void closeStream(int streamId, ChannelHandlerContext ctx) {
            Http2TargetHandler clientOutboundHandler =
                    (Http2TargetHandler) ctx.pipeline().get(netty.contract.Constants.HTTP2_TARGET_HANDLER);
        ...
        private void notifyTimeoutError(OutboundMsgHolder msgHolder, boolean primary) {
            if (primary) {
                try {
                    ...
                    msgHolder.getResponseFuture().notifyHttpListener(new netty.contract.exceptions.EndpointTimeOutException(
                ...
                msgHolder.getResponseFuture().notifyPushResponse(streamId, new EndpointTimeOutException(
                        netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_BEFORE_INITIATING_PUSH_RESPONSE,
                        HttpResponseStatus.GATEWAY_TIMEOUT.code()));
            }
        }
