...
public class ReceivingEntityBody implements SenderState {
    ...
    @Override
    public void readInboundResponseBody(ChannelHandlerContext ctx, netty.message.Http2DataFrame http2DataFrame,
                                        OutboundMsgHolder outboundMsgHolder, boolean serverPush,
    ...
    private void onServerPushDataRead(OutboundMsgHolder outboundMsgHolder, int streamId, boolean endOfStream,
                                      ByteBuf data) {
        netty.message.HttpCarbonMessage responseMessage = outboundMsgHolder.getPushResponse(streamId);
        if (endOfStream) {
