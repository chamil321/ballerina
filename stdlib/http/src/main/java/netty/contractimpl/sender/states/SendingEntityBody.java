...
public class SendingEntityBody implements SenderState {
    ...
    public void handleAbruptChannelClosure(TargetHandler targetHandler, netty.contract.HttpResponseFuture httpResponseFuture) {
        targetHandler.getOutboundRequestMsg()
                .setIoException(new IOException(
                        netty.contract.Constants.REMOTE_SERVER_CLOSED_WHILE_WRITING_OUTBOUND_REQUEST_BODY));
        httpResponseFuture.notifyHttpListener(
                        new ClientConnectorException(senderReqRespStateManager.nettyTargetChannel.id().asShortText(),
                                                     netty.contract.Constants.REMOTE_SERVER_CLOSED_WHILE_WRITING_OUTBOUND_REQUEST_BODY));
        LOG.error("Error in HTTP client: {}", netty.contract.Constants.REMOTE_SERVER_CLOSED_WHILE_WRITING_OUTBOUND_REQUEST_BODY);
    }
    ...
    public void handleIdleTimeoutConnectionClosure(TargetHandler targetHandler,
        ...
        targetHandler.getOutboundRequestMsg()
                .setIoException(new IOException(
                        netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_WRITING_OUTBOUND_REQUEST_BODY));
        httpResponseFuture.notifyHttpListener(
                new EndpointTimeOutException(channelID, netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_WRITING_OUTBOUND_REQUEST_BODY,
                                             HttpResponseStatus.INTERNAL_SERVER_ERROR.code()));

        LOG.error("Error in HTTP client: {}", netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_WRITING_OUTBOUND_REQUEST_BODY);
    }
        ...
        outboundRequestChannelFuture.addListener(writeOperationPromise -> {
            ...
            if (throwable != null) {
                if (throwable instanceof ClosedChannelException) {
                    throwable = new IOException(netty.contract.Constants.CLIENT_TO_REMOTE_HOST_CONNECTION_CLOSED);
                }
