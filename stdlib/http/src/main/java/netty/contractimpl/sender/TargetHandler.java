...
import netty.contractimpl.common.states.Http2StateUtil;
import netty.contractimpl.common.states.SenderReqRespStateManager;
import netty.contractimpl.sender.channel.TargetChannel;
import netty.contractimpl.sender.channel.pool.ConnectionManager;
import netty.contractimpl.sender.http2.Http2ClientChannel;
import netty.contractimpl.sender.http2.Http2ClientTimeoutHandler;
...
import netty.message.ClientRemoteFlowControlListener;
...
public class TargetHandler extends ChannelInboundHandlerAdapter {
    ...
    private netty.contract.HttpResponseFuture httpResponseFuture;
    private netty.message.HttpCarbonMessage inboundResponseMsg;
    private netty.contractimpl.sender.channel.pool.ConnectionManager connectionManager;
    private netty.contractimpl.sender.channel.TargetChannel targetChannel;
    private netty.contractimpl.sender.http2.Http2TargetHandler http2TargetHandler;
    private netty.message.HttpCarbonMessage outboundRequestMsg;
        ...
        if (targetChannel.isRequestHeaderWritten()) {
            if (msg instanceof HttpResponse) {
                if (isAbnormal100Response((HttpResponse) msg)) {
                    ...
                    inboundResponseMsg = Util.createInboundRespCarbonMsg(ctx, (HttpResponse) msg, outboundRequestMsg);
    ...
    private void executePostUpgradeActions(ChannelHandlerContext ctx) {
        ...
        ctx.pipeline().addLast(netty.contract.Constants.HTTP2_TARGET_HANDLER, http2TargetHandler);
        netty.contractimpl.sender.http2.Http2ClientChannel http2ClientChannel = http2TargetHandler.getHttp2ClientChannel();

        // Remove Http specific handlers
        Util.safelyRemoveHandlers(targetChannel.getChannel().pipeline(), netty.contract.Constants.IDLE_STATE_HANDLER,
                                  netty.contract.Constants.HTTP_TRACE_LOG_HANDLER);
        Http2StateUtil.initHttp2MessageContext(outboundRequestMsg, http2TargetHandler);
    ...
    public void setHttpResponseFuture(netty.contract.HttpResponseFuture httpResponseFuture) {
        this.httpResponseFuture = httpResponseFuture;
    }

    public void setConnectionManager(netty.contractimpl.sender.channel.pool.ConnectionManager connectionManager) {
        this.connectionManager = connectionManager;
    }

    public void setOutboundRequestMsg(netty.message.HttpCarbonMessage outboundRequestMsg) {
        this.outboundRequestMsg = outboundRequestMsg;
    }

    public void setTargetChannel(netty.contractimpl.sender.channel.TargetChannel targetChannel) {
        this.targetChannel = targetChannel;
    }

    public void setKeepAliveConfig(netty.contract.config.KeepAliveConfig keepAliveConfig) {
    ...
    void setHttp2TargetHandler(netty.contractimpl.sender.http2.Http2TargetHandler http2TargetHandler) {
        this.http2TargetHandler = http2TargetHandler;
    }

    public netty.message.HttpCarbonMessage getInboundResponseMsg() {
