...
public class SendingHeaders implements SenderState {
    ...
    private final netty.contract.config.ChunkConfig chunkConfig;
    ...
    public SendingHeaders(SenderReqRespStateManager senderReqRespStateManager,
                          ...
                          netty.contract.config.ChunkConfig chunkConfig, netty.contract.HttpResponseFuture httpInboundResponseFuture) {
    ...
    public void writeOutboundRequestEntity(netty.message.HttpCarbonMessage httpOutboundRequest, HttpContent httpContent) {
        if (netty.contractimpl.common.Util.isLastHttpContent(httpContent)) {
            if (netty.contractimpl.common.Util.checkContentLengthAndTransferEncodingHeaderAllowance(httpOutboundRequest)) {
                if (chunkConfig == netty.contract.config.ChunkConfig.ALWAYS && checkChunkingCompatibility(httpVersion, chunkConfig)) {
                    netty.contractimpl.common.Util.setupChunkedRequest(httpOutboundRequest);
                } else {
                    contentLength += httpContent.content().readableBytes();
                    netty.contractimpl.common.Util.setupContentLengthRequest(httpOutboundRequest, contentLength);
                }
            }
            ...
            if ((chunkConfig == netty.contract.config.ChunkConfig.ALWAYS || chunkConfig == ChunkConfig.AUTO) &&
    ...
    private void writeRequestHeaders(netty.message.HttpCarbonMessage httpOutboundRequest,
                                     netty.contract.HttpResponseFuture httpInboundResponseFuture, String httpVersion,
                                     TargetChannel targetChannel) {
        setHttpVersionProperty(httpOutboundRequest, httpVersion);
        HttpRequest httpRequest = netty.contractimpl.common.Util.createHttpRequest(httpOutboundRequest);
        ...
        notifyIfHeaderWriteFailure(httpInboundResponseFuture, outboundHeaderFuture,
                                   netty.contract.Constants.CLIENT_TO_REMOTE_HOST_CONNECTION_CLOSED);
    }

    private void setHttpVersionProperty(netty.message.HttpCarbonMessage httpOutboundRequest, String httpVersion) {
        if (netty.contract.Constants.HTTP_2_0.equals(httpVersion)) {
            // Upgrade request of HTTP/2 should be a HTTP/1.1 request
            httpOutboundRequest.setHttpVersion(String.valueOf(netty.contract.Constants.HTTP_1_1));
        } else {
            httpOutboundRequest.setHttpVersion(httpVersion);
        }
    }

    private void writeRequestBody(netty.message.HttpCarbonMessage httpOutboundRequest, HttpContent httpContent) {
        String expectHeader = httpOutboundRequest.getHeader(HttpHeaderNames.EXPECT.toString());
        if (expectHeader != null && expectHeader.equalsIgnoreCase(netty.contract.Constants.HEADER_VAL_100_CONTINUE)) {
    ...
    public void readInboundResponseHeaders(TargetHandler targetHandler, HttpResponse httpInboundResponse) {
        ...
        targetHandler.getOutboundRequestMsg().setIoException(new IOException(
                netty.contract.Constants.INBOUND_RESPONSE_ALREADY_RECEIVED));
    ...
    public void handleAbruptChannelClosure(TargetHandler targetHandler, netty.contract.HttpResponseFuture httpResponseFuture) {
        ...
        LOG.error(netty.contract.Constants.REMOTE_SERVER_CLOSED_WHILE_WRITING_OUTBOUND_REQUEST_HEADERS);
    }
    ...
    public void handleIdleTimeoutConnectionClosure(TargetHandler targetHandler,
        ...
        // HttpResponseFuture will be notified asynchronously via writeOutboundRequestHeaders method.
        senderReqRespStateManager.nettyTargetChannel.pipeline().remove(netty.contract.Constants.IDLE_STATE_HANDLER);
        senderReqRespStateManager.nettyTargetChannel.close();
        LOG.error("Error in HTTP client: {}", netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_WRITING_OUTBOUND_REQUEST_HEADERS);
    }
}
