...
import netty.message.BackPressureObservable;
import netty.message.DefaultBackPressureObservable;
...
public class OutboundMsgHolder {
    ...
    private netty.message.HttpCarbonMessage requestCarbonMessage;
    private BlockingQueue<netty.message.Http2PushPromise> promises;
    private ConcurrentHashMap<Integer, netty.message.HttpCarbonResponse> pushResponsesMap;
    private netty.message.HttpCarbonResponse response;

    // Future which is used to notify the response listener upon response receive
    private netty.contract.HttpResponseFuture responseFuture;
    ...
    private final netty.message.BackPressureObservable backPressureObservable = new DefaultBackPressureObservable();

    public OutboundMsgHolder(netty.message.HttpCarbonMessage httpOutboundRequest) {
    ...
    /**
     * Gets the associated {@link org.wso2.transport.http.netty.contractimpl.sender.http2.Http2ClientChannel}.
     *
    ...
    /**
     * Adds a {@link netty.message.Http2PushPromise} message.
     *
     * @param pushPromise push promise message
     */
    public void addPromise(netty.message.Http2PushPromise pushPromise) {
     ...
     */
    public void addPushResponse(int streamId, netty.message.HttpCarbonResponse pushResponse) {
     ...
     */
    public netty.message.HttpCarbonResponse getPushResponse(int steamId) {
     ...
     */
    public netty.message.HttpCarbonResponse getResponse() {
        return response;
    }
