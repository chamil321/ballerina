...
public class ForwardedHeaderUpdater {
    ...
    private final netty.message.HttpCarbonMessage httpOutboundRequest;
    ...
    public ForwardedHeaderUpdater(HttpCarbonMessage httpOutboundRequest, String localAddress) {
        ...
        this.forwardedHeader = httpOutboundRequest.getHeader(netty.contract.Constants.FORWARDED);
        this.xForwardedForHeader = httpOutboundRequest.getHeader(netty.contract.Constants.X_FORWARDED_FOR);
        this.xForwardedByHeader = httpOutboundRequest.getHeader(netty.contract.Constants.X_FORWARDED_BY);
        this.xForwardedHostHeader = httpOutboundRequest.getHeader(netty.contract.Constants.X_FORWARDED_HOST);
        this.xForwardedProtoHeader = httpOutboundRequest.getHeader(netty.contract.Constants.X_FORWARDED_PROTO);
    }
    ...
    public void setForwardedHeader() {
        ...
        if (forwardedHeader == null) {
            Object remoteAddressProperty = httpOutboundRequest.getProperty(netty.contract.Constants.REMOTE_ADDRESS);
            ...
            Object hostHeader = httpOutboundRequest.getProperty(netty.contract.Constants.ORIGIN_HOST);
            if (hostHeader != null) {
                headerValue.append(SEMI_COLON + " " + HOST).append(hostHeader.toString());
            }
            Object protocolHeader = httpOutboundRequest.getProperty(netty.contract.Constants.PROTOCOL);
            if (protocolHeader != null) {
                headerValue.append(SEMI_COLON + " " + PROTO).append(protocolHeader.toString());
            }
            httpOutboundRequest.setHeader(netty.contract.Constants.FORWARDED, headerValue.toString());
            return;
        }
    ...
    public void setDefactoForwardedHeaders() {
        if (xForwardedForHeader != null && xForwardedByHeader != null) {
            httpOutboundRequest.setHeader(netty.contract.Constants.X_FORWARDED_FOR,
                                          xForwardedForHeader.trim() + COMMA + " " + xForwardedByHeader.trim());
        } else if (xForwardedByHeader != null) {
            httpOutboundRequest.setHeader(netty.contract.Constants.X_FORWARDED_FOR, xForwardedByHeader.trim());
        }
