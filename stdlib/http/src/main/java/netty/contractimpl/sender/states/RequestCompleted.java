...
public class RequestCompleted implements SenderState {
    ...
    public void handleAbruptChannelClosure(TargetHandler targetHandler, netty.contract.HttpResponseFuture httpResponseFuture) {
        httpResponseFuture.notifyHttpListener(
                new ServerConnectorException(
                        netty.contract.Constants.REMOTE_SERVER_CLOSED_BEFORE_INITIATING_INBOUND_RESPONSE));
        LOG.error(netty.contract.Constants.REMOTE_SERVER_CLOSED_BEFORE_INITIATING_INBOUND_RESPONSE);
    }
    ...
    public void handleIdleTimeoutConnectionClosure(TargetHandler targetHandler,
        ...
        httpResponseFuture.notifyHttpListener(
                new EndpointTimeOutException(channelID, netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_BEFORE_INITIATING_INBOUND_RESPONSE,
                                             HttpResponseStatus.GATEWAY_TIMEOUT.code()));
        LOG.error("Error in HTTP client: {}", netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_BEFORE_INITIATING_INBOUND_RESPONSE);
    }
}
