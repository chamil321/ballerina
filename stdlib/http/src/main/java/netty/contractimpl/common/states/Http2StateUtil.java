...
package netty.contractimpl.common.states;
...
import netty.contractimpl.Http2OutboundRespListener;
import netty.contractimpl.common.Util;
import netty.contractimpl.listener.http2.Http2SourceHandler;
import netty.contractimpl.listener.http2.InboundMessageHolder;
import netty.contractimpl.listener.states.http2.Response100ContinueSent;
import netty.contractimpl.listener.states.http2.SendingHeaders;
import netty.contractimpl.sender.http2.Http2DataEventListener;
import netty.contractimpl.sender.http2.Http2TargetHandler;
import netty.contractimpl.sender.states.http2.RequestCompleted;
import netty.message.Http2DataFrame;
import netty.message.Http2InboundContentListener;
import netty.message.Http2PushPromise;
import netty.message.Http2Reset;
import netty.message.HttpCarbonMessage;
import netty.message.HttpCarbonRequest;
...
public class Http2StateUtil {
     ...
     */
    public static void notifyRequestListener(netty.contractimpl.listener.http2.Http2SourceHandler http2SourceHandler,
                                             InboundMessageHolder inboundMessageHolder, int streamId) {
        netty.message.HttpCarbonMessage httpRequestMsg = inboundMessageHolder.getInboundMsg();
        if (http2SourceHandler.getServerConnectorFuture() != null) {
            try {
                ServerConnectorFuture outboundRespFuture = httpRequestMsg.getHttpResponseFuture();
                netty.contractimpl.Http2OutboundRespListener
                        http2OutboundRespListener = new netty.contractimpl.Http2OutboundRespListener(
     ...
     */
    public static netty.message.HttpCarbonRequest setupCarbonRequest(HttpRequest httpRequest, netty.contractimpl.listener.http2.Http2SourceHandler http2SourceHandler,
                                                                     int streamId) {
        ChannelHandlerContext ctx = http2SourceHandler.getChannelHandlerContext();
        netty.message.HttpCarbonRequest sourceReqCMsg = new HttpCarbonRequest(httpRequest, new Http2InboundContentListener(
                streamId, ctx, http2SourceHandler.getConnection(), netty.contract.Constants.INBOUND_REQUEST));
        sourceReqCMsg.setProperty(netty.contract.Constants.POOLED_BYTE_BUFFER_FACTORY, new PooledDataStreamerFactory(ctx.alloc()));
        sourceReqCMsg.setProperty(netty.contract.Constants.CHNL_HNDLR_CTX, ctx);
        sourceReqCMsg.setProperty(netty.contract.Constants.SRC_HANDLER, http2SourceHandler);
        ...
        sourceReqCMsg.setProperty(netty.contract.Constants.LOCAL_ADDRESS, localAddress);
        sourceReqCMsg.setProperty(netty.contract.Constants.REMOTE_ADDRESS, http2SourceHandler.getRemoteAddress());
        sourceReqCMsg.setProperty(netty.contract.Constants.LISTENER_PORT, localAddress != null ? localAddress.getPort() : null);
        sourceReqCMsg.setProperty(netty.contract.Constants.LISTENER_INTERFACE_ID, http2SourceHandler.getInterfaceId());
        sourceReqCMsg.setProperty(netty.contract.Constants.PROTOCOL, netty.contract.Constants.HTTP_SCHEME);
        sourceReqCMsg.setProperty(netty.contract.Constants.MUTUAL_SSL_HANDSHAKE_RESULT,
                                  ctx.channel().attr(netty.contract.Constants.MUTUAL_SSL_RESULT_ATTRIBUTE).get());
        sourceReqCMsg.setProperty(netty.contract.Constants.BASE_64_ENCODED_CERT,
                                  ctx.channel().attr(netty.contract.Constants.BASE_64_ENCODED_CERT_ATTRIBUTE).get());
        String uri = httpRequest.uri();
        sourceReqCMsg.setRequestUrl(uri);
        sourceReqCMsg.setProperty(netty.contract.Constants.TO, uri);
    ...
    public static void writeHttp2ResponseHeaders(ChannelHandlerContext ctx, Http2ConnectionEncoder encoder,
                                                 netty.contract.HttpResponseFuture outboundRespStatusFuture, int streamId,
                                                 Http2Headers http2Headers, boolean endStream,
                                                 netty.contractimpl.Http2OutboundRespListener respListener) throws Http2Exception {
        for (netty.contractimpl.sender.http2.Http2DataEventListener dataEventListener : respListener.getHttp2ServerChannel().getDataEventListeners()) {
        ...
        ctx.flush();
        netty.contractimpl.common.Util.addResponseWriteFailureListener(outboundRespStatusFuture, channelFuture, respListener);
    }
     ...
     * @throws Http2Exception throws if a protocol-related error occurred
     */
    public static void writeHttp2Promise(netty.message.Http2PushPromise pushPromise, ChannelHandlerContext ctx, Http2Connection conn,
                                         Http2ConnectionEncoder encoder, netty.message.HttpCarbonMessage inboundRequestMsg,
        ...
        httpRequest.headers().add(HttpConversionUtil.ExtensionHeaderNames.SCHEME.text(), netty.contract.Constants.HTTP_SCHEME);
        ...
        encoder.flowController().writePendingBytes();
        ctx.flush();
        netty.contractimpl.common.Util.checkForResponseWriteStatus(inboundRequestMsg, outboundRespStatusFuture, channelFuture);
    }
    ...
    public static void validatePromisedStreamState(int originalStreamId, int streamId, Http2Connection conn,
                                                   netty.message.HttpCarbonMessage inboundRequestMsg) throws Http2Exception {
        ...
        if (!isValidStreamId(streamId, conn)) {
            inboundRequestMsg.getHttpOutboundRespStatusFuture().
                    notifyHttpListener(new ServerConnectorException(
                            netty.contract.Constants.PROMISED_STREAM_REJECTED_ERROR));
            throw new Http2Exception(Http2Error.REFUSED_STREAM, netty.contract.Constants.PROMISED_STREAM_REJECTED_ERROR);
        }
    }
    ...
    public static void releaseDataFrame(Http2SourceHandler http2SourceHandler, Http2DataFrame dataFrame) {
        ...
        netty.message.HttpCarbonMessage sourceReqCMsg = http2SourceHandler.getStreamIdRequestMap().get(streamId).getInboundMsg();
    ...
    public static void writeHttp2Headers(ChannelHandlerContext ctx, OutboundMsgHolder outboundMsgHolder,
        ...
        for (netty.contractimpl.sender.http2.Http2DataEventListener dataEventListener : http2ClientChannel.getDataEventListeners()) {
            if (!dataEventListener.onHeadersWrite(ctx, streamId, http2Headers, endStream)) {
                return;
            }
        }
    ...
    public static void sendRequestTimeoutResponse(ChannelHandlerContext ctx,
                                                  netty.contractimpl.Http2OutboundRespListener http2OutboundRespListener,
    ...
    private static void handleFuture(ChannelFuture outboundRespFuture, boolean handleIncompleteRequest,
                                     boolean whileReceivingHeader, netty.message.HttpCarbonMessage inboundRequestMsg) {
        outboundRespFuture.addListener((ChannelFutureListener) channelFuture -> {
            ...
            if (handleIncompleteRequest) {
                if (whileReceivingHeader) {
                    StateUtil.handleIncompleteInboundMessage(inboundRequestMsg,
                                                             netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_READING_INBOUND_REQUEST_HEADERS);
                } else {
                    StateUtil.handleIncompleteInboundMessage(inboundRequestMsg,
                                                             netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_READING_INBOUND_REQUEST_BODY);
                }
            }
        });
    }

    public static void initHttp2MessageContext(netty.message.HttpCarbonMessage outboundRequest,
        ...
        if (http2MessageStateContext == null) {
            ...
            http2MessageStateContext.setSenderState(new netty.contractimpl.sender.states.http2.RequestCompleted(http2TargetHandler, null));
