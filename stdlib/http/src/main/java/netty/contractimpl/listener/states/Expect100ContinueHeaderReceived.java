...
public class Expect100ContinueHeaderReceived implements ListenerState {
    ...
    Expect100ContinueHeaderReceived(ListenerReqRespStateManager listenerReqRespStateManager,
                                    SourceHandler sourceHandler, netty.message.HttpCarbonMessage inboundRequestMsg, float httpVersion) {
    ...
    @Override
    public void readInboundRequestBody(Object inboundRequestEntityBody) throws
        ...
        try {
            serverConnectorFuture.notifyErrorListener(
                    new netty.contract.exceptions.ServerConnectorException(
                            netty.contract.Constants.REMOTE_CLIENT_CLOSED_BEFORE_INITIATING_100_CONTINUE_RESPONSE));
    ...
    public ChannelFuture handleIdleTimeoutConnectionClosure(ServerConnectorFuture serverConnectorFuture,
        ...
        try {
            serverConnectorFuture.notifyErrorListener(
                    new netty.contract.exceptions.ServerConnectorException(
                            netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_BEFORE_INITIATING_100_CONTINUE_RESPONSE));
    ...
    private ChannelFuture sendRequestTimeoutResponse(ChannelHandlerContext ctx, HttpResponseStatus status,
        ...
        outboundResponse.headers().set(HttpHeaderNames.CONTENT_TYPE, netty.contract.Constants.TEXT_PLAIN);
        outboundResponse.headers().set(HttpHeaderNames.CONNECTION.toString(), netty.contract.Constants.CONNECTION_CLOSE);
