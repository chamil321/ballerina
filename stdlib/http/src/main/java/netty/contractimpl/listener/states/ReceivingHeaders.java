...
public class ReceivingHeaders implements ListenerState {
    ...
    private void notifyRequestListener(netty.message.HttpCarbonMessage httpRequestMsg) {
        if (handlerExecutor != null) {
        ...
        if (sourceHandler.getServerConnectorFuture() != null) {
            try {
                netty.contract.ServerConnectorFuture outboundRespFuture = httpRequestMsg.getHttpResponseFuture();
                outboundRespFuture.setHttpConnectorListener(
                        new netty.contractimpl.HttpOutboundRespListener(httpRequestMsg, sourceHandler));
    ...
    public void handleAbruptChannelClosure(netty.contract.ServerConnectorFuture serverConnectorFuture) {
        handleIncompleteInboundMessage(inboundRequestMsg, Constants.REMOTE_CLIENT_CLOSED_WHILE_READING_INBOUND_REQUEST_HEADERS);
    }
    ...
    public ChannelFuture handleIdleTimeoutConnectionClosure(ServerConnectorFuture serverConnectorFuture,
        ...
        outboundRespFuture.addListener((ChannelFutureListener) channelFuture -> {
            ...
            handleIncompleteInboundMessage(inboundRequestMsg,
                                           Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_READING_INBOUND_REQUEST_HEADERS);
        });
        listenerReqRespStateManager.state
