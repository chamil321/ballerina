...
public class EntityBodyReceived implements ListenerState {
    ...
    public void handleStreamTimeout(netty.contract.ServerConnectorFuture serverConnectorFuture, ChannelHandlerContext ctx,
        ...
        } catch (netty.contract.exceptions.ServerConnectorException e) {
            LOG.error(Constants.IDLE_TIMEOUT_TRIGGERED_BEFORE_INITIATING_OUTBOUND_RESPONSE + ":" + e.getMessage());
        }
        ...
        Http2StateUtil.sendRequestTimeoutResponse(ctx, http2OutboundRespListener, streamId,
                        ...
                        Constants.HTTP2_SERVER_TIMEOUT_ERROR_MESSAGE, CharsetUtil.UTF_8), false, false);
    }
    ...
    public void handleAbruptChannelClosure(ServerConnectorFuture serverConnectorFuture, ChannelHandlerContext ctx,
        ...
        http2OutboundRespListener.getOutboundResponseMsg().setIoException(
                new IOException(Constants.REMOTE_CLIENT_CLOSED_BEFORE_INITIATING_OUTBOUND_RESPONSE));
        try {
            serverConnectorFuture.notifyErrorListener(
                    new ClientClosedConnectionException(
