...
package netty.contractimpl.listener.http2;
...
import netty.contractimpl.listener.states.http2.EntityBodyReceived;
import netty.contractimpl.listener.states.http2.ReceivingHeaders;
...
import netty.message.ServerRemoteFlowControlListener;
 ...
 * <p>
 * This is also responsible for building the {@link netty.message.HttpCarbonRequest} and forward to the listener
...
public final class Http2SourceHandler extends ChannelInboundHandlerAdapter {
    ...
    private netty.message.ServerRemoteFlowControlListener serverRemoteFlowControlListener;
    private SocketAddress remoteAddress;

    Http2SourceHandler(HttpServerChannelInitializer serverChannelInitializer, Http2ConnectionEncoder encoder,
                       String interfaceId, Http2Connection conn, netty.contract.ServerConnectorFuture serverConnectorFuture,
                       String serverName) {
    ...
    private void setDataEventListeners() {
        long serverTimeout = serverChannelInitializer.getSocketIdleTimeout() <= 0 ? netty.contract.Constants.ENDPOINT_TIMEOUT :
                serverChannelInitializer.getSocketIdleTimeout();
        http2ServerChannel.addDataEventListener(netty.contract.Constants.IDLE_STATE_HANDLER,
    ...
    private void setRemoteFlowController() {
        ...
        serverRemoteFlowControlListener = new netty.message.ServerRemoteFlowControlListener(remoteFlowController);
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) {
        if (evt instanceof HttpServerUpgradeHandler.UpgradeEvent) {
            ...
            HttpRequest httpRequest = new DefaultHttpRequest(
                    new HttpVersion(netty.contract.Constants.HTTP_VERSION_2_0, true), upgradedRequest.method(),
                    upgradedRequest.uri(), upgradedRequest.headers());

            HttpCarbonRequest requestCarbonMessage = setupCarbonRequest(httpRequest, this, netty.contract.Constants.STREAM_ID_ONE);
            ...
            http2ServerChannel.getStreamIdRequestMap().put(netty.contract.Constants.STREAM_ID_ONE, inboundMsgHolder);
            http2ServerChannel.getDataEventListeners()
                    .forEach(dataEventListener -> dataEventListener.onStreamInit(ctx, netty.contract.Constants.STREAM_ID_ONE));
            notifyRequestListener(this, inboundMsgHolder, netty.contract.Constants.STREAM_ID_ONE);
        }
    }

    @Override
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Http2Exception {
        if (msg instanceof netty.message.Http2HeadersFrame) {
            netty.message.Http2HeadersFrame headersFrame = (Http2HeadersFrame) msg;
            Http2MessageStateContext http2MessageStateContext = new Http2MessageStateContext();
        ...
        } else if (msg instanceof netty.message.Http2DataFrame) {
            netty.message.Http2DataFrame dataFrame = (Http2DataFrame) msg;
            int streamId = dataFrame.getStreamId();
            netty.message.HttpCarbonMessage sourceReqCMsg = http2ServerChannel.getInboundMessage(streamId)
                    .getInboundMsg();
