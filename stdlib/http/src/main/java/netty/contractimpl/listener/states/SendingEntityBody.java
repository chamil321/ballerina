...
public class SendingEntityBody implements ListenerState {
    ...
    private final netty.contract.HttpResponseFuture outboundRespStatusFuture;
    ...
    private netty.message.HttpCarbonMessage outboundResponseMsg;
    private ChannelHandlerContext sourceContext;
    private SourceHandler sourceHandler;

    SendingEntityBody(ListenerReqRespStateManager listenerReqRespStateManager,
                      netty.contract.HttpResponseFuture outboundRespStatusFuture, boolean headersWritten) {
    ...
    @Override
    public void writeOutboundResponseBody(netty.contractimpl.HttpOutboundRespListener outboundRespListener,
                                          netty.message.HttpCarbonMessage outboundResponseMsg, HttpContent httpContent) {

        headRequest = outboundRespListener.getRequestDataHolder().getHttpMethod().equalsIgnoreCase(
                netty.contract.Constants.HTTP_HEAD_METHOD);
    ...
    public void handleAbruptChannelClosure(netty.contract.ServerConnectorFuture serverConnectorFuture) {
        IOException connectionClose = new IOException(
                netty.contract.Constants.REMOTE_CLIENT_CLOSED_WHILE_WRITING_OUTBOUND_RESPONSE_BODY);
        outboundResponseMsg.setIoException(connectionClose);
        outboundRespStatusFuture.notifyHttpListener(connectionClose);

        LOG.error(netty.contract.Constants.REMOTE_CLIENT_CLOSED_WHILE_WRITING_OUTBOUND_RESPONSE_BODY);
    }
    ...
    public ChannelFuture handleIdleTimeoutConnectionClosure(ServerConnectorFuture serverConnectorFuture,
        ...
        IOException connectionClose = new IOException(
                netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_WRITING_OUTBOUND_RESPONSE_BODY);
        outboundResponseMsg.setIoException(connectionClose);
        outboundRespStatusFuture.notifyHttpListener(connectionClose);

        LOG.error(netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_WRITING_OUTBOUND_RESPONSE_BODY);
        return null;
    }
    ...
    private ChannelFuture writeOutboundResponseHeaderAndBody(HttpOutboundRespListener outboundRespListener,
                                                             netty.message.HttpCarbonMessage outboundResponseMsg,
        ...
        HttpResponse fullOutboundResponse = Util.createFullHttpResponse(outboundResponseMsg,
    ...
    private void checkForResponseWriteStatus(netty.message.HttpCarbonMessage inboundRequestMsg,
        ...
        channelFuture.addListener(writeOperationPromise -> {
            ...
            if (throwable != null) {
                if (throwable instanceof ClosedChannelException) {
                    throwable = new IOException(netty.contract.Constants.REMOTE_CLIENT_TO_HOST_CONNECTION_CLOSED);
                }
    ...
    private void triggerPipeliningLogic(HttpCarbonMessage outboundResponseMsg) {
        ...
        if (outboundResponseMsg.isPipeliningEnabled() && netty.contract.Constants.HTTP_1_1_VERSION.equalsIgnoreCase
                (httpVersion)) {
            Queue responseQueue;
            synchronized (sourceContext.channel().attr(netty.contract.Constants.RESPONSE_QUEUE).get()) {
                responseQueue = sourceContext.channel().attr(netty.contract.Constants.RESPONSE_QUEUE).get();
                Long nextSequenceNumber = sourceContext.channel().attr(netty.contract.Constants.NEXT_SEQUENCE_NUMBER).get();
                ...
                nextSequenceNumber++;
                sourceContext.channel().attr(netty.contract.Constants.NEXT_SEQUENCE_NUMBER).set(nextSequenceNumber);
            ...
            if (!responseQueue.isEmpty()) {
                ...
                if (outboundResponseMsg.getPipeliningFuture() != null) {
                    EventExecutorGroup pipeliningExecutor = sourceContext.channel().attr(netty.contract.Constants.PIPELINING_EXECUTOR)
                            .get();
