...
package netty.contractimpl.listener;
...
import netty.contract.config.RequestSizeValidationConfig;
import netty.contractimpl.common.certificatevalidation.CertificateVerificationException;
...
import netty.contractimpl.listener.http2.Http2SourceConnectionHandlerBuilder;
import netty.contractimpl.listener.http2.Http2ToHttpFallbackHandler;
import netty.contractimpl.listener.http2.Http2WithPriorKnowledgeHandler;
...
public class HttpServerChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    private netty.contract.config.ChunkConfig chunkConfig;
    ...
    private netty.contract.ServerConnectorFuture serverConnectorFuture;
    private netty.contract.config.RequestSizeValidationConfig reqSizeValidationConfig;
        ...
        if (http2Enabled) {
            if (sslHandlerFactory != null) {
                if (ocspStaplingEnabled) {
                    ...
                    serverPipeline
                            .addLast(sslHandler, new Http2PipelineConfiguratorForServer(this, sslHandler.engine()));
                    serverPipeline.addLast(netty.contract.Constants.HTTP2_EXCEPTION_HANDLER, new Http2ExceptionHandler());
                }
            } else {
            ...
            if (sslHandlerFactory != null) {
                ...
                configureHttpPipeline(serverPipeline, netty.contract.Constants.HTTP_SCHEME);
            }
        }
    }
    ...
    private OCSPResp getOcspResponse()
            throws IOException, KeyStoreException,
                   netty.contractimpl.common.certificatevalidation.CertificateVerificationException, CertificateException {
    ...
    private void configureSslForHttp(ChannelPipeline serverPipeline, SocketChannel ch)
        ...
        if (ocspStaplingEnabled) {
            ...
            netty.contractimpl.common.Util.setSslHandshakeTimeOut(sslConfig, sslHandler);
            serverPipeline.addLast(netty.contract.Constants.SSL_HANDLER, sslHandler);
            if (validateCertEnabled) {
                serverPipeline.addLast(netty.contract.Constants.HTTP_CERT_VALIDATION_HANDLER,
                                       new CertificateValidationHandler(sslEngine, cacheDelay, cacheSize));
            }
        }
        serverPipeline.addLast(netty.contract.Constants.SSL_COMPLETION_HANDLER,
    ...
    public void configureHttpPipeline(ChannelPipeline serverPipeline, String initialHttpScheme) {
        ...
        if (initialHttpScheme.equals(netty.contract.Constants.HTTP_SCHEME)) {
            serverPipeline.addLast(netty.contract.Constants.HTTP_ENCODER, new HttpResponseEncoder());
            serverPipeline.addLast(netty.contract.Constants.HTTP_DECODER,
            ...
            serverPipeline.addLast(netty.contract.Constants.HTTP_COMPRESSOR, new CustomHttpContentCompressor());
            serverPipeline.addLast(netty.contract.Constants.HTTP_CHUNK_WRITER, new ChunkedWriteHandler());

            if (httpTraceLogEnabled) {
                serverPipeline.addLast(netty.contract.Constants.HTTP_TRACE_LOG_HANDLER, new HttpTraceLoggingHandler(
                        netty.contract.Constants.TRACE_LOG_DOWNSTREAM));
            }
            if (httpAccessLogEnabled) {
                serverPipeline.addLast(netty.contract.Constants.HTTP_ACCESS_LOG_HANDLER, new HttpAccessLoggingHandler(
                        netty.contract.Constants.ACCESS_LOG));
            }
        }
        ...
        serverPipeline.addLast(netty.contract.Constants.WEBSOCKET_SERVER_HANDSHAKE_HANDLER,
                               new WebSocketServerHandshakeHandler(this.serverConnectorFuture,
                                                                   webSocketCompressionEnabled));
        serverPipeline.addLast(netty.contract.Constants.BACK_PRESSURE_HANDLER, new BackPressureHandler());
        serverPipeline.addLast(netty.contract.Constants.HTTP_SOURCE_HANDLER,
        ...
        if (socketIdleTimeout >= 0) {
            serverPipeline.addBefore(netty.contract.Constants.HTTP_SOURCE_HANDLER, netty.contract.Constants.IDLE_STATE_HANDLER,
                                     new IdleStateHandler(0, 0, socketIdleTimeout, TimeUnit.MILLISECONDS));
        }
        serverPipeline.addLast(netty.contract.Constants.HTTP_EXCEPTION_HANDLER, new HttpExceptionHandler());
    }
    ...
    private void configureH2cPipeline(ChannelPipeline pipeline) {
        ...
        final HttpServerUpgradeHandler.UpgradeCodecFactory upgradeCodecFactory = protocol -> {
            if (AsciiString.contentEquals(Http2CodecUtil.HTTP_UPGRADE_PROTOCOL_NAME, protocol)) {
                return new Http2ServerUpgradeCodec(
                        netty.contract.Constants.HTTP2_SOURCE_CONNECTION_HANDLER,
                        new netty.contractimpl.listener.http2.Http2SourceConnectionHandlerBuilder(
                                interfaceId, serverConnectorFuture, serverName, this).build());
            } else {
                return null;
            }
        };
        pipeline.addLast(netty.contract.Constants.HTTP_SERVER_CODEC, sourceCodec);
        pipeline.addLast(netty.contract.Constants.HTTP_COMPRESSOR, new CustomHttpContentCompressor());
        if (httpTraceLogEnabled) {
            pipeline.addLast(netty.contract.Constants.HTTP_TRACE_LOG_HANDLER,
                             new HttpTraceLoggingHandler(netty.contract.Constants.TRACE_LOG_DOWNSTREAM));
        }
        if (httpAccessLogEnabled) {
            pipeline.addLast(netty.contract.Constants.HTTP_ACCESS_LOG_HANDLER, new HttpAccessLoggingHandler(
                    netty.contract.Constants.ACCESS_LOG));
        }
        pipeline.addLast(netty.contract.Constants.HTTP2_UPGRADE_HANDLER,
        ...
        pipeline.addLast(netty.contract.Constants.HTTP2_TO_HTTP_FALLBACK_HANDLER,
    ...
    class Http2PipelineConfiguratorForServer extends ApplicationProtocolNegotiationHandler {
        ...
        protected void configurePipeline(ChannelHandlerContext ctx, String protocol) {
            netty.contractimpl.common.Util.setMutualSslStatus(ctx, sslEngine);
            if (ApplicationProtocolNames.HTTP_2.equals(protocol)) {
                ...
                configureHttpPipeline(ctx.pipeline(), netty.contract.Constants.HTTP_SCHEME);
