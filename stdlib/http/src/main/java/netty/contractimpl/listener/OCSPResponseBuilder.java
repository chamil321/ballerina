...
import netty.contractimpl.common.certificatevalidation.Constants;
import netty.contractimpl.common.certificatevalidation.ocsp.OCSPCache;
import netty.contractimpl.common.certificatevalidation.ocsp.OCSPVerifier;
...
public class OCSPResponseBuilder {
    ...
    static OCSPResp generateOcspResponse(netty.contractimpl.common.ssl.SSLConfig sslConfig, int cacheAllocatedSize, int cacheDelay)
            throws IOException, KeyStoreException,
                   netty.contractimpl.common.certificatevalidation.CertificateVerificationException, CertificateException {

        int cacheSize = netty.contractimpl.common.certificatevalidation.Constants.CACHE_DEFAULT_ALLOCATED_SIZE;
        int cacheDelayMins = netty.contractimpl.common.certificatevalidation.Constants.CACHE_DEFAULT_DELAY_MINS;

        if (cacheAllocatedSize != 0 && cacheAllocatedSize > netty.contractimpl.common.certificatevalidation.Constants.CACHE_MIN_ALLOCATED_SIZE
                && cacheAllocatedSize < netty.contractimpl.common.certificatevalidation.Constants.CACHE_MAX_ALLOCATED_SIZE) {
            cacheSize = cacheAllocatedSize;
        }
        if (cacheDelay != 0 && cacheDelay > netty.contractimpl.common.certificatevalidation.Constants.CACHE_MIN_DELAY_MINS
                && cacheDelay < Constants.CACHE_MAX_DELAY_MINS) {
            cacheDelayMins = cacheDelay;
        }

        netty.contractimpl.common.certificatevalidation.ocsp.OCSPCache
                ocspCache = netty.contractimpl.common.certificatevalidation.ocsp.OCSPCache.getCache();
        ...
        if (userCertificate == null) {
            throw new netty.contractimpl.common.certificatevalidation.CertificateVerificationException("Could not get revocation status from OCSP.");
        } else {
            ...
            if (ocspCache.getOCSPCacheValue(userCertificate.getSerialNumber()) != null) {
                ...
                OCSPReq request;
                request = netty.contractimpl.common.certificatevalidation.ocsp.OCSPVerifier.generateOCSPRequest(issuer, userCertificate.getSerialNumber());
     ...
     * @throws netty.contractimpl.common.certificatevalidation.CertificateVerificationException If an error occurs while getting the AIA locations from the certificate.
     */
    public static List<String> getAIALocations(X509Certificate userCertificate)
            throws netty.contractimpl.common.certificatevalidation.CertificateVerificationException {
        List<String> locations;
        //List the AIA locations from the certificate. Those are the URL's of CA s.
        try {
            locations = netty.contractimpl.common.certificatevalidation.ocsp.OCSPVerifier
                    .getAIALocations(userCertificate);
        } catch (netty.contractimpl.common.certificatevalidation.CertificateVerificationException e) {
            throw new netty.contractimpl.common.certificatevalidation.CertificateVerificationException("Failed to find AIA locations in the cetificate", e);
        }
        return locations;
    }
     ...
     * @throws netty.contractimpl.common.certificatevalidation.CertificateVerificationException If an error occurs while getting the response.
     */
    public static OCSPResp getOCSPResponse(List<String> locations, OCSPReq request, X509Certificate userCertificate,
            OCSPCache ocspCache) throws
                                 netty.contractimpl.common.certificatevalidation.CertificateVerificationException {
        ...
        for (String serviceUrl : locations) {
            ...
            } catch (OCSPException | netty.contractimpl.common.certificatevalidation.CertificateVerificationException e) {
