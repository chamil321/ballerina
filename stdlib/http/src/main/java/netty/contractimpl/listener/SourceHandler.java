...
import netty.contract.exceptions.ClientClosedConnectionException;
import netty.contract.exceptions.ServerConnectorException;
import netty.contractimpl.common.Util;
import netty.contractimpl.listener.states.ListenerReqRespStateManager;
import netty.contractimpl.listener.states.ReceivingHeaders;
import netty.internal.HandlerExecutor;
import netty.internal.HttpTransportContextHolder;
...
public class SourceHandler extends ChannelInboundHandlerAdapter {
    ...
    private netty.message.HttpCarbonMessage inboundRequestMsg;
    private final Map<Integer, netty.message.HttpCarbonMessage> requestSet = new ConcurrentHashMap<>();
    ...
    private final Queue holdingQueue = new PriorityQueue<>(netty.contract.Constants.NUMBER_OF_INITIAL_EVENTS_HELD);
    private EventExecutorGroup pipeliningGroup;

    public SourceHandler(netty.contract.ServerConnectorFuture serverConnectorFuture, String interfaceId, netty.contract.config.ChunkConfig chunkConfig,
                         netty.contract.config.KeepAliveConfig keepAliveConfig, String serverName, ChannelGroup allChannels, boolean
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof HttpRequest) {
            ...
            inboundRequestMsg = Util.createInboundReqCarbonMsg((HttpRequest) msg, ctx, this);
            ...
            netty.contractimpl.listener.states.ListenerReqRespStateManager
        ...
        if (!idleTimeout) {
            if (!requestSet.isEmpty()) {
                ...
                notifyErrorListenerAtConnectedState(
                        netty.contract.Constants.REMOTE_CLIENT_CLOSED_BEFORE_INITIATING_INBOUND_REQUEST);
            }
        }
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) {
        if (evt instanceof IdleStateEvent) {
            ...
            if (!requestSet.isEmpty()) {
                ...
                if (connectedState) {
                    notifyErrorListenerAtConnectedState(
                            netty.contract.Constants.IDLE_TIMEOUT_TRIGGERED_BEFORE_INITIATING_INBOUND_REQUEST);
                }
            }
    ...
    private void setRequestProperties() {
        ...
        inboundRequestMsg.setKeepAlive(Util.isKeepAliveConnection(keepAliveConfig, connectionHeaderValue,
                                                                  httpVersion));
    }
    ...
    private void setPipeliningProperties() {
        if (ctx.channel().attr(netty.contract.Constants.MAX_RESPONSES_ALLOWED_TO_BE_QUEUED).get() == null) {
            ctx.channel().attr(netty.contract.Constants.MAX_RESPONSES_ALLOWED_TO_BE_QUEUED).set(pipeliningLimit);
        }
        if (ctx.channel().attr(netty.contract.Constants.RESPONSE_QUEUE).get() == null) {
            ctx.channel().attr(netty.contract.Constants.RESPONSE_QUEUE).set(holdingQueue);
        }
        if (ctx.channel().attr(netty.contract.Constants.NEXT_SEQUENCE_NUMBER).get() == null) {
            ctx.channel().attr(netty.contract.Constants.NEXT_SEQUENCE_NUMBER).set(
                    netty.contract.Constants.EXPECTED_SEQUENCE_NUMBER);
        }

        if (ctx.channel().attr(netty.contract.Constants.PIPELINING_EXECUTOR).get() == null) {
            ctx.channel().attr(netty.contract.Constants.PIPELINING_EXECUTOR).set(pipeliningGroup);
        }
    }
