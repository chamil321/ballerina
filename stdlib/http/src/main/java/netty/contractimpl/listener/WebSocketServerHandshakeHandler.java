...
import netty.contractimpl.websocket.message.DefaultWebSocketHandshaker;
...
public class WebSocketServerHandshakeHandler extends ChannelInboundHandlerAdapter {
    ...
    private final netty.contract.ServerConnectorFuture serverConnectorFuture;
    ...
    private ChannelHandlerContext modifyPipelineForUpgrade(ChannelHandlerContext ctx) {
        ...
        if (pipeline.get(netty.contract.Constants.BACK_PRESSURE_HANDLER) != null) {
            pipeline.remove(netty.contract.Constants.BACK_PRESSURE_HANDLER);
        }
        // Retain a reference for the sourceHandler to create a carbon message
        sourceHandler = (SourceHandler) pipeline.remove(netty.contract.Constants.HTTP_SOURCE_HANDLER);
        ChannelHandlerContext decoderCtx = pipeline.context(HttpRequestDecoder.class);
        pipeline.addAfter(decoderCtx.name(), netty.contract.Constants.HTTP_OBJECT_AGGREGATOR,
                          new HttpObjectAggregator(netty.contract.Constants.WEBSOCKET_REQUEST_SIZE));
        if (webSocketCompressionEnabled) {
            pipeline.addAfter(netty.contract.Constants.HTTP_OBJECT_AGGREGATOR, netty.contract.Constants.WEBSOCKET_COMPRESSION_HANDLER,
                              new WebSocketServerCompressionHandler());
            pipeline.addAfter(netty.contract.Constants.WEBSOCKET_COMPRESSION_HANDLER, Utf8FrameValidator.class.getName(),
                              new Utf8FrameValidator());
        } else {
            pipeline.addAfter(netty.contract.Constants.HTTP_OBJECT_AGGREGATOR, Utf8FrameValidator.class.getName(),
                              new Utf8FrameValidator());
        }
    ...
    private class HandShakeHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
        ...
        private void handleWebSocketHandshake(FullHttpRequest fullHttpRequest, ChannelHandlerContext ctx)
            ...
            netty.contractimpl.websocket.message.DefaultWebSocketHandshaker webSocketHandshaker =
