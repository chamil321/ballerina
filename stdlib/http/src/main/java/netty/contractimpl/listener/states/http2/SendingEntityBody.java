...
package netty.contractimpl.listener.states.http2;
...
public class SendingEntityBody implements ListenerState {
    ...
    private final netty.message.HttpCarbonMessage inboundRequestMsg;
    private final Calendar inboundRequestArrivalTime;
    private final int originalStreamId;
    private final netty.contractimpl.Http2OutboundRespListener http2OutboundRespListener;
    private netty.message.HttpCarbonMessage outboundResponseMsg;

    private Long contentLength = 0L;
    private String remoteAddress;

    SendingEntityBody(netty.contractimpl.Http2OutboundRespListener http2OutboundRespListener,
    ...
    @Override
    public void writeOutboundResponseHeaders(netty.contractimpl.Http2OutboundRespListener http2OutboundRespListener,
    ...
    @Override
    public void writeOutboundResponseBody(netty.contractimpl.Http2OutboundRespListener http2OutboundRespListener,
                                          netty.message.HttpCarbonMessage outboundResponseMsg, HttpContent httpContent,
    ...
    @Override
    public void writeOutboundPromise(netty.contractimpl.Http2OutboundRespListener http2OutboundRespListener,
    ...
    @Override
    public void handleStreamTimeout(netty.contract.ServerConnectorFuture serverConnectorFuture, ChannelHandlerContext ctx,
                                    netty.contractimpl.Http2OutboundRespListener http2OutboundRespListener, int streamId) {
        try {
            serverConnectorFuture.notifyErrorListener(
                    new netty.contract.exceptions.ServerConnectorException(
                            Constants.IDLE_TIMEOUT_TRIGGERED_WHILE_WRITING_OUTBOUND_RESPONSE_BODY));
    ...
    public void handleAbruptChannelClosure(ServerConnectorFuture serverConnectorFuture, ChannelHandlerContext ctx,
        ...
        IOException connectionClose = new IOException(
                Constants.REMOTE_CLIENT_CLOSED_WHILE_WRITING_OUTBOUND_RESPONSE_BODY);
    ...
    private void writeContent(Http2OutboundRespListener http2OutboundRespListener,
                              netty.message.HttpCarbonMessage outboundResponseMsg, HttpContent httpContent, int streamId)
    ...
    private void writeData(HttpContent httpContent, int streamId, boolean endStream) throws Http2Exception {
        ...
        if (endStream) {
            ...
            netty.contractimpl.common.Util.checkForResponseWriteStatus(inboundRequestMsg, outboundRespStatusFuture,
                                                                       channelFuture);
        } else {
            netty.contractimpl.common.Util.addResponseWriteFailureListener(outboundRespStatusFuture, channelFuture, http2OutboundRespListener);
        }
    }
