...
public class Http2SourceConnectionHandler extends Http2ConnectionHandler {
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        ...
        Util.safelyRemoveHandlers(ctx.pipeline(), netty.contract.Constants.HTTP2_TO_HTTP_FALLBACK_HANDLER,
                                  netty.contract.Constants.HTTP_COMPRESSOR,
                                  netty.contract.Constants.HTTP_TRACE_LOG_HANDLER, netty.contract.Constants.HTTP_ACCESS_LOG_HANDLER, netty.contract.Constants.HTTP2_EXCEPTION_HANDLER);
        ...
        ctx.pipeline().addLast(netty.contract.Constants.HTTP2_SOURCE_HANDLER, http2SourceHandler);
        ctx.pipeline().addLast(netty.contract.Constants.HTTP2_EXCEPTION_HANDLER, new Http2ExceptionHandler(this));
    }
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        ...
        if (netty.internal.HttpTransportContextHolder.getInstance().getHandlerExecutor() != null) {
    ...
    private static class ServerFrameListener extends Http2EventAdapter {
        ...
        public void onHeadersRead(ChannelHandlerContext ctx, int streamId,
            ...
            netty.message.Http2HeadersFrame http2HeadersFrame = new Http2HeadersFrame(streamId, headers, endOfStream);
        ...
        public int onDataRead(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding, boolean endOfStream) {
            ...
            netty.message.Http2DataFrame dataFrame = new Http2DataFrame(streamId, forwardedData, endOfStream);
            ctx.fireChannelRead(dataFrame);
            return netty.contract.Constants.ZERO_READABLE_BYTES;
        }
