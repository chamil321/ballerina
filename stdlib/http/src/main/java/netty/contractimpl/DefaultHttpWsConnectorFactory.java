...
import netty.contract.config.ListenerConfiguration;
import netty.contract.config.SenderConfiguration;
import netty.contract.config.ServerBootstrapConfiguration;
import netty.contract.websocket.WebSocketClientConnector;
...
public class DefaultHttpWsConnectorFactory implements HttpWsConnectorFactory {
    ...
    public ServerConnector createServerConnector(ServerBootstrapConfiguration serverBootstrapConfiguration,
                                                 netty.contract.config.ListenerConfiguration listenerConfig) {
        ...
        if (netty.contract.Constants.HTTP_2_0.equals(listenerConfig.getVersion())) {
        ...
        if (listenerConfig.isPipeliningEnabled()) {
            pipeliningGroup = new DefaultEventExecutorGroup(netty.contract.Constants.PIPELINING_THREAD_COUNT, new DefaultThreadFactory(
                    netty.contract.Constants.PIPELINING_THREAD_POOL_NAME));
    ...
    public HttpClientConnector createHttpClientConnector(
            Map<String, Object> transportProperties, netty.contract.config.SenderConfiguration senderConfiguration) {
